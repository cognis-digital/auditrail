"""Native cognis-connect emit for auditrail — forward findings to any platform.

Maps auditrail's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    auditrail ... --format json | auditrail-emit --to stix
    auditrail-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('action', '') + ' by ' + rec.get('actor', '')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('type', 'activity')
